...
export default function MonthView({ currentDate, exams, onDateClick, onEventClick, selectedDates = [], isMultiSelectMode = false, isReadOnly = false }: MonthViewProps) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        <div
          ...
          className={`relative flex flex-col rounded-[8px] md:rounded-[10px] min-[2000px]:rounded-[16px] p-0.5 md:p-1 min-[2000px]:p-2 transition-all duration-200 cursor-pointer shadow-sm min-h-0 ${!isMultiSelectMode ? 'group hover:z-50' : ''} ${
            isCurrentMonth 
              ? isSelected 
        ...
        >
          {isSelected && (
            <CheckCircle2 className="absolute top-1 right-1 w-3 h-3 md:top-1.5 md:right-1.5 md:w-3.5 md:h-3.5 min-[2000px]:top-2 min-[2000px]:right-2 min-[2000px]:w-5 min-[2000px]:h-5 text-[#EF4444] fill-red-100" />
          )}
          <div className="flex justify-center items-center h-4 md:h-5 min-[2000px]:h-6 w-full mb-0.5 min-[2000px]:mb-1 shrink-0">
            <span
              className={`font-bold text-[10px] md:text-[11px] min-[2000px]:text-[15px] ${
          ...
          <div className={`flex-1 flex flex-col justify-start content-start gap-[2px] md:gap-[3px] min-[2000px]:gap-1.5 overflow-y-auto no-scrollbar min-h-0 ${!isMultiSelectMode ? 'md:group-hover:opacity-0 transition-opacity duration-200' : ''}`}>
            {dayExams.map((exam) => (
              <div
                key={exam.id}
                onClick={(e) => {
                ...
                className="cursor-pointer rounded-[3px] md:rounded-[4px] min-[2000px]:rounded-md bg-red-100/80 px-0.5 py-[1px] md:px-1 md:py-[2px] min-[2000px]:px-1.5 min-[2000px]:py-1 hover:bg-red-200 transition-colors flex flex-col items-center text-center w-full min-w-0"
              >
                <span className="font-bold text-[7.5px] md:text-[8px] min-[2000px]:text-[12px] text-[#EF4444] line-clamp-2 break-words w-full leading-[1.1]">
                  {isReadOnly ? (exam.labs?.length ? exam.labs.join(", ") : "Occupied") : exam.examName}
                </span>
                {!isReadOnly && (
                  <span className="hidden md:block font-semibold text-[6.5px] md:text-[7px] min-[2000px]:text-[10px] text-[#DC2626] truncate w-full leading-[1.1] mt-[1px]">
                    {exam.shifts} shifts
                  </span>
                )}
              </div>
  ...
  return (
    <div className="flex h-full flex-col min-h-0">
      <div className="grid grid-cols-7 gap-1 md:gap-2 min-[2000px]:gap-4 mb-1 md:mb-2 min-[2000px]:mb-4">
        {weekDays.map((wd, i) => (
          <div 
            key={i} 
            className={`bg-white rounded-[8px] md:rounded-[12px] min-[2000px]:rounded-[16px] py-1.5 md:py-2 min-[2000px]:py-4 text-center text-[9px] md:text-[10px] min-[2000px]:text-xs font-black shadow-sm ${
      ...
      <div 
        className="grid flex-1 grid-cols-7 gap-1 md:gap-2 min-[2000px]:gap-4 min-h-0"
