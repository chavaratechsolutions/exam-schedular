...
export default function ExamListView({ exams, onEdit, onDelete, isReadOnly = false }: ExamListViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="bg-white px-4 md:px-5 2xl:px-8 py-3 md:py-4 2xl:py-6 rounded-[12px] md:rounded-[16px] 2xl:rounded-[24px] shadow-sm mb-3 md:mb-4 2xl:mb-6 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-3 md:gap-4">
        <div className="text-center sm:text-left">
          <h2 className="text-base md:text-lg 2xl:text-2xl font-black text-gray-800 tracking-tight">
            {filterType === "upcoming" ? "Upcoming Exams" : "All Scheduled Exams"}
          </h2>
          <p className="text-gray-500 text-[10px] md:text-xs 2xl:text-sm font-medium mt-0.5 md:mt-1">Showing {filteredExams.length} exam{filteredExams.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {filteredExams.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar min-h-0 pb-4 md:pb-6 pr-2">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 2xl:gap-6">
            {filteredExams.map((exam) => {
              const isPast = exam.date.getTime() < today.getTime();
              return (
              <div 
                key={exam.id}
                className={`p-3 md:p-4 2xl:p-6 rounded-[12px] md:rounded-[16px] 2xl:rounded-[24px] shadow-sm hover:shadow-md transition-shadow border-2 flex flex-col min-h-[160px] md:min-h-[180px] 2xl:min-h-[220px] ${
                  isPast
              ...
              >
                <div className="flex justify-between items-start mb-2 md:mb-3 2xl:mb-4">
                  <h3 className="font-bold text-[#EF4444] text-base md:text-lg 2xl:text-2xl leading-tight line-clamp-2" title={isReadOnly ? format(exam.date, "EEEE, MMMM d, yyyy") : exam.examName}>
                ...
                <div className="flex flex-col gap-2 md:gap-2.5 2xl:gap-3 flex-1">
                  ...
                  {exam.labs && exam.labs.length > 0 && !isReadOnly && (
                    <div className="flex flex-wrap gap-1 md:gap-1.5 p-2 md:p-2.5 2xl:p-3 bg-gray-50 rounded-[10px] md:rounded-[12px] 2xl:rounded-[16px] border border-gray-100">
                      {exam.labs.map((lab) => (
                        <span key={lab} className="px-1.5 py-0.5 md:px-2 md:py-1 2xl:px-2.5 2xl:py-1 text-[8px] md:text-[9px] 2xl:text-[11px] font-bold bg-white text-gray-700 rounded-[4px] md:rounded-[6px] 2xl:rounded-[8px] border border-gray-200 shadow-sm">
                          {lab}
                        </span>
